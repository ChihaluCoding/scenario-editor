...
export function SceneMetadata({ scene }: { scene: Scene }) {
  ...
  return (
    <div className="grid grid-cols-2 gap-2 lg:grid-cols-[1fr_1fr_1.4fr_1fr]">
      <label className="flex flex-col gap-1 text-[11px] font-medium text-ink-400">
        チャプター
        <input
          value={scene.chapter}
      ...
      <label className="flex flex-col gap-1 text-[11px] font-medium text-ink-400">
        制作状態
        <select value={scene.status} onChange={(event) => updateScene(scene.id, { status: event.target.value as Scene['status'] })} className="field-input cursor-pointer text-[12px]">
          {SCENE_STATUSES.map((status) => <option key={status} value={status}>{SCENE_STATUS_LABEL[status]}</option>)}
        </select>
      </label>
      <label className="flex flex-col gap-1 text-[11px] font-medium text-ink-400">
        タグ（カンマ区切り）
        <input value={tags} onChange={(event) => setTags(event.target.value)} onBlur={commitTags} onKeyDown={(event) => event.key === 'Enter' && event.currentTarget.blur()} placeholder="共通, 日常" className="field-input text-[12px]" />
      </label>
      <label className="flex flex-col gap-1 text-[11px] font-medium text-ink-400">
        エンディング
        <select value={scene.ending ?? ''} onChange={(event) => updateScene(scene.id, { ending: (event.target.value || null) as Scene['ending'] })} className="field-input cursor-pointer text-[12px]">

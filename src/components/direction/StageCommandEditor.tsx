...
export function StageCommandEditor({ line, onChange }: { line: StageLine; onChange: (patch: Partial<StageLine>) => void }) {
  ...
  return (
    <div className="flex flex-col gap-2 rounded-lg border border-line-stage/25 bg-line-stage/6 p-2.5">
      <div className="flex flex-wrap items-center gap-2">
        <select value={line.action} onChange={(event) => onChange({ action: event.target.value as StageLine['action'] })} className="field-input w-48 cursor-pointer text-xs">
        ...
        {(line.action === 'background' || line.action === 'character') && (
          <select value={line.transition} onChange={(event) => onChange({ transition: event.target.value as StageLine['transition'] })} className="field-input w-32 cursor-pointer text-xs">
      ...
      {line.action === 'character' && (
        <div className="grid grid-cols-1 gap-2 lg:grid-cols-[1fr_1fr_7rem]">
          <select value={line.charId} onChange={(event) => onChange({ charId: event.target.value, portraitId: '' })} className="field-input cursor-pointer text-xs">
            <option value="">キャラクターを選択</option>
            {characters.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}
          </select>
          <select value={line.portraitId} onChange={(event) => onChange({ portraitId: event.target.value })} className="field-input cursor-pointer text-xs">
            <option value="">通常立ち絵</option>
            {character?.portraits.map((portrait) => <option key={portrait.id} value={portrait.id}>{portrait.name}</option>)}
          </select>
          <select value={line.position} onChange={(event) => onChange({ position: event.target.value as StageLine['position'] })} className="field-input cursor-pointer text-xs">
      ...
      {line.action === 'hide' && (
        <select value={line.position} onChange={(event) => onChange({ position: event.target.value as StageLine['position'] })} className="field-input w-40 cursor-pointer text-xs">
      ...
      {line.action === 'effect' && (
        <div className="flex items-center gap-2">
          <select value={line.screenEffect} onChange={(event) => onChange({ screenEffect: event.target.value as StageLine['screenEffect'] })} className="field-input w-44 cursor-pointer text-xs">
...
function DurationInput({ value, onChange }: { value: number; onChange: (value: number) => void }) {
  return (
    <label className="flex items-center gap-1.5 text-xs text-ink-400">
      時間
      <input type="number" min={0} step={100} value={value} onChange={(event) => onChange(Math.max(0, Number(event.target.value) || 0))} className="field-input w-24 text-xs" />
      ms
    </label>
  )
}

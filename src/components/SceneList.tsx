...
import { Button, IconButton, PanelHeading } from './ui'
...
function SceneRow({ scene, index }: { scene: Scene; index: number }) {
  ...
  return (
    <li
      ...
      className={clsx(
        'group flex items-center gap-1.5 rounded-lg border px-1.5 py-1.5 transition',
        selected ? 'border-brand/60 bg-brand/12' : 'border-transparent hover:bg-ink-800',
    ...
    >
      ...
      <button onClick={() => selectScene(scene.id)} className="flex min-w-0 flex-1 flex-col items-start text-left">
        <span className="flex w-full items-center gap-1.5">
          {isStart && <Play size={10} className="shrink-0 fill-good text-good" />}
          <span className="truncate font-medium">{scene.title || '(無題)'}</span>
        ...
        <span className="flex items-center gap-1.5 text-[11px] text-ink-400">
          <span className={clsx('size-1.5 rounded-full', scene.status === 'done' ? 'bg-good' : scene.status === 'review' ? 'bg-warn' : scene.status === 'writing' ? 'bg-brand' : 'bg-ink-400')} />
      ...
      <div className="flex shrink-0 items-center opacity-0 transition group-hover:opacity-100">
        {!isStart && (
          <IconButton label="開始シーンにする" variant="ghost" onClick={() => setStartScene(scene.id)}>
            <Play size={13} />
          </IconButton>
        )}
        <IconButton label="複製" variant="ghost" onClick={() => duplicateScene(scene.id)}>
          <Copy size={13} />
        </IconButton>
        <IconButton
          label="削除"
...
export function SceneList() {
  ...
  return (
    <section>
      <PanelHeading
        action={
          <Button variant="ghost" onClick={addScene} className="px-1.5 py-0.5 text-ink-300">
            <Plus size={14} />
          </Button>
        }
      >
      ...
      <div className="mb-2 flex flex-col gap-1.5">
        <div className="flex items-center gap-2 text-[10px] text-ink-400">
          <div className="h-1.5 flex-1 overflow-hidden rounded-full bg-ink-700">
            <div className="h-full rounded-full bg-good transition-all" style={{ width: `${scenes.length ? (done / scenes.length) * 100 : 0}%` }} />
          </div>
          <span>{done}/{scenes.length} 完成</span>
        </div>
        <div className="grid grid-cols-2 gap-1">
          <select value={chapterFilter} onChange={(event) => setChapterFilter(event.target.value)} className="field-input cursor-pointer px-1.5 py-1 text-[11px]">
            <option value="">全チャプター</option>
            {chapters.map((chapter) => <option key={chapter} value={chapter}>{chapter}</option>)}
          </select>
          <select value={statusFilter} onChange={(event) => setStatusFilter(event.target.value as '' | SceneStatus)} className="field-input cursor-pointer px-1.5 py-1 text-[11px]">
            <option value="">全制作状態</option>
            {SCENE_STATUSES.map((status) => <option key={status} value={status}>{SCENE_STATUS_LABEL[status]}</option>)}
          </select>
        </div>
        <input value={tagFilter} onChange={(event) => setTagFilter(event.target.value)} placeholder="タグで絞り込み" className="field-input px-1.5 py-1 text-[11px]" />
      </div>
              ...
              return (
                <div key={chapter}>
                  <button
                    ...
                    className="mb-0.5 flex w-full items-center gap-1 px-1 text-left text-[10px] font-semibold tracking-wide text-ink-400 uppercase hover:text-ink-100"
                  >
